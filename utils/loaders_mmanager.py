...
class LoadersManager:
    def __init__(self, loaders: Dict[str, Type] | None = None):
        """Менеджер загрузки документов"""
    ...
    def load_documents(self, path: str) -> List[Document]:
        ...
        for file in Path(path).rglob("*.*"):
            ...
            try:
                ...
                loader = self.loaders[ext](str(file))
                ...
                for meta_ext in [".meta.json", ".meta.yaml", "meta.yml"]:
                    ...
                    if meta_path.exists():
                        custom_metadata = self._load_metadata(meta_path)
                        logger.debug(
